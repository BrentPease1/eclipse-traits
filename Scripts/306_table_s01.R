# make a table with common and scientific names for species included in analyses
library(tidyverse)
library(here)
library(flextable)
library(officer)
...
sp_table <- bin_df_to |> 
  ...
  full_join(bin_df_an |>
              dplyr::select(com, sci) |>
              distinct() |>
              add_column(annular = 1)) |>
  mutate(eclipse = ifelse(total == 1 & annular == 1, "both", NA)) |>
  mutate(eclipse = ifelse(is.na(eclipse) & total == 1, "total", eclipse)) |>
  mutate(eclipse = ifelse(is.na(eclipse) & annular == 1, "annular", eclipse)) |>
  dplyr::select(common = com, scientific = sci, eclipse) 

ft <- flextable::flextable(sp_table)  
