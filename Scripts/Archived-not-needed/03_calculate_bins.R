...
( raw_plot <- bin_df |> 
  ...
  ggplot(aes(x = bin_rel, y = val)) +
  geom_point(size = 3,
             color = MetBrewer::MetPalettes$Isfahan1[[1]][c(6)]) +
  geom_line(linewidth = 0.5, 
            color = MetBrewer::MetPalettes$Isfahan1[[1]][c(6)]) +
  labs(x = "time from totality (min)",
       y = "number of species–site combinations\nwith detections during 4-minute window") +
  theme_classic() +
...
unique_sites <- birdnet |> 
  dplyr::filter(date == lubridate::ymd("2024-04-08")) |> 
  dplyr::select(lat = Latitude, lon = Longitude, MaxEclipseTimeUTC) |> 
  dplyr::distinct() |> 
  ...
  dplyr::left_join(
    final |> 
      ...
      dplyr::select(
        ...
        bin_rel) |>
      dplyr::distinct()) |>
  dplyr::mutate( period = ifelse(bin_rel < -8, "pre", ifelse( bin_rel > 8, "post", "tot"))) |> 
...
ggplot() +
  geom_line(data = sp, 
            aes(x = bin_rel, y = fit, group = com, color = type2)) +
  facet_wrap(~type2)
...
( gs_plot <- ggplot() +
    geom_line(data = filter(sp, !(com %in% c("Barred Owl", "Tufted Titmouse")) & ma < 0.4),
              ...
              alpha = 0.2) +
    geom_line(data = filter(sp, com %in% c("Barred Owl","Tufted Titmouse")),
...
lp <- mgcv::bam(
  y ~ s(bin.sc, by = alan, k = 8, bs = "tp") + s(sp.station, bs = "re"),
  family = binomial(link = "logit"), 
  data = final)

et <- mgcv::bam(
  y ~ s(bin.sc, by = et, k = 8, bs = "tp") + s(sp.station, bs = "re"),
  family = binomial(link = "logit"), 
  data = final)
...
et_nd <- tidyr::expand_grid(bin.sc = sort(unique(final$bin.sc)), et = c(min(final$et), max(final$et))) 
...
( dnc_plot <- ggplot( data = filter(all_p, predictor == "diel niche (continuous)"),
        aes(x = bin_rel, y = fit, color = factor(cov), fill = factor(cov))) +
  facet_wrap(~predictor) +
  ylim(c(0, 0.20)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, color = NA) +
  geom_line(size = 2) +
  theme_classic() +
  scale_color_manual(values = cp(4)) +
  scale_fill_manual(values = cp(4)) +
  labs(x = "time from totality (min)",
       y = "probability of vocalization during 4-minute window") +
  theme(axis.line = element_line(color = "black", linewidth = 0.2), 
        axis.ticks = element_line(color = "black", linewidth = 0.2), 
...
( mi_plot <- ggplot( data = filter(all_p, predictor == "migration"),
                     aes(x = bin_rel, y = fit, color = factor(cov), fill = factor(cov))) +
    facet_wrap(~predictor) +
    ylim(c(0, 0.20)) +
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, color = NA) +
    geom_line(size = 2) +
    theme_classic() +
    scale_color_manual(values = mi_cp(3)) +
    scale_fill_manual(values = mi_cp(3)) +
    labs(x = "time from totality (min)",
         y = "probability of vocalization during 4-minute window") +
    theme(axis.line = element_line(color = "black", linewidth = 0.2), 
          axis.ticks = element_line(color = "black", linewidth = 0.2), 
