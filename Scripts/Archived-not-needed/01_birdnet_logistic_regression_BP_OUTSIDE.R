...
# Filter within_path
dt_in_path <- birdnet[within_path == "outside_path"]

# Filter focal time detections
focal_time_detections <- dt_in_path[
  start_time >= FirstContactTimeUTC & start_time <= FourthContactTimeUTC &
...
setnames(test, c("2024-04-07", "2024-04-08", "2024-04-09"), c("tm1", "t", "tp1"), skip_absent = T)
...
lots_of_dets <- det[, .N, by = common][N >= 100]
...
m1 <- glmmTMB::glmmTMB(
  y ~ 1 + factor(eclipse) + (1 + factor(eclipse) | common),
  data = filtered_det,
...
ggsave(plot = last_plot(), filename = here('Results/Figures/prob_of_vocal_eclipse_outside.png'),
...
# trying again
ggplot(preds_sorted, aes(x=species, y=difference, label=difference)) + 
  geom_bar(stat='identity', aes(fill=direction), width=.5) + 
  scale_fill_manual(name="Effect of Eclipse", 
                    labels = c("Increased Vocalization", "Decreased Vocalization"), 
                    values = c("Increased"="gold", "Decreased"="black")) + 
  labs(y="Probability of Vocalization") + 
  coord_flip()
...
ggsave(plot = gg, filename = here('Results/Figures/prob_of_vocal_eclipse_species_outside.png'),
       width = 8, height = 8, dpi = 300)
...
gg <- ggplot(pred_probs, aes(x=prob_eclipse_0 , xend=prob_eclipse_1, y=reorder(families, prob_eclipse_1), group=families))
gg <- gg + geom_dumbbell(colour="gold", size=3, colour_xend="black", 
                         dot_guide=T, dot_guide_size=0.15)
...
ggsave(plot = gg, filename = here('Results/Figures/prob_of_vocal_eclipse_family_outside.png'),
       width = 8, height = 8, dpi = 300)
