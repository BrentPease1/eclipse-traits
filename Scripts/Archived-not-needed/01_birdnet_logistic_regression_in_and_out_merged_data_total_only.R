...
birdnet <- fread(here('Data/merged_es_bw_apr2025_viirs.csv'))


# Read data
birdnet <- fread(here::here("Data/birdnet_cleaned_v03.csv"))
# fix timestamps
time_cols <- c('start_time', "FirstContactTimeUTC",
...
setnames(elton, old = 'English', new = 'common_name') 
# !! missing rows !! (FIXED!)
...
birdnet <- merge(birdnet, elton[, .(BLFamilyLatin, common_name)], by = "common_name", allow.cartesian=TRUE)
setnames(birdnet, old = 'BLFamilyLatin', new = 'family')
birdnet <- birdnet[!duplicated(quick_index),]

# trying to get time around eclipse for detections outside of path
birdnet[, end_of_eclipse := as_hms(MaxEclipseTimeUTC + as_hms('00:02:00'))]
birdnet[, start_of_eclipse := as_hms(MaxEclipseTimeUTC - as_hms('00:02:00'))]

# Filter within_path
# dt_in_path <- birdnet[within_path == "within_path"]

# Filter focal time detections
focal_time_detections <- birdnet[
  start_time >= start_of_eclipse & start_time <= end_of_eclipse &
...
test <- unique(focal_time_detections[, .(station, common, sci, family, activity, path_status, date)])
test[, y := 1]

# Reshape test data: retain detections
test <- dcast(test, station + common + sci + family + activity + path_status ~ date, value.var = "y", fill = 0)
setnames(test, c("2024-04-07", "2024-04-08", "2024-04-09"), c("tm1", "t", "tp1"))

# Create template (all potential species-station combinations)
template <- unique(birdnet[date %in% ymd(c("2024-04-07", "2024-04-08", "2024-04-09")), 
                              .(station = ESIDNumber, common = common_name, 
                                sci = scientific_name, activity, family, path_status)])

# Merge test (detections) with template (all spp) 
det <- merge(template, test, by = c("station", "common", "sci", "family", "activity", "path_status"), all.x = TRUE)
...
# Reshape long format
det <- melt(det, id.vars = c("station", "common", "sci", "family", "activity", "path_status"), 
...
det[, eclipse_fac := as.factor(eclipse)]
...
lots_of_dets <- det[, .N, by = common][N >= 50]
...
m1 <- glmmTMB::glmmTMB(
  y ~ 1 + eclipse_fac + path_status + activity + (1 + eclipse_fac| common),
  data = filtered_det,
  family = binomial(link = "logit")) 

#
m2 <- glmmTMB::glmmTMB(
  y ~ 1 + eclipse_fac + path_status + (1 + eclipse_fac| common),
  data = filtered_det,
  family = binomial(link = "logit"),
) 
...
m4 <- glmmTMB::glmmTMB(
  y ~ 1 + eclipse_fac*path_status + (1 + eclipse_fac| common) + (1 | station),
  data = filtered_det,
  family = binomial(link = "logit"),
) 
AIC(m1, m2, m3, m4) #m4 is best

# generate prediction
# Get predicted probabilities across eclipse 
preds <- ggpredict(m4, terms = c("eclipse_fac"))
...
ggsave(plot = last_plot(), filename = here('Results/Figures/prob_of_vocal_eclipse_merged_may2025.png'),
...
## -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
# !! !!
# getting probability of vocalization for fixed + random effects for each spp from m4
## -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
## -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --

# Extract fixed and random effects
fixef_m4_cond <- fixef(m4)$cond
ranef_common <- ranef(m4, condVar = TRUE)$cond$common

# Prepare combinations
pred_probs <- expand.grid(
  species = rownames(ranef_common),
  eclipse_fac = c(0, 1),
  path_status = c("within_path", "outside_path"),
  stringsAsFactors = FALSE
)
pred_probs$probability <- NA

# Loop through and calculate log-odds and probabilities
for (i in 1:nrow(pred_probs)) {
  sp <- pred_probs$species[i]
  eclipse <- pred_probs$eclipse_fac[i]
  path <- pred_probs$path_status[i]
  
  ranef_vals <- ranef_common[sp, ]
  intercept <- ranef_vals[1]
  slope_eclipse <- ranef_vals[2]
  
  log_odds <- fixef_m4_cond["(Intercept)"] + intercept
  
  # Add eclipse effect if eclipse == 1
  if (eclipse == 1) {
    log_odds <- log_odds + fixef_m4_cond["eclipse_fac1"] + ranef_vals[2]
  }
  
  # Add path_status effect if within_path (ref level is outside_path)
  if (path == "within_path") {
    log_odds <- log_odds + fixef_m4_cond["path_statuswithin_path"]
    
    # Add interaction only if eclipse also == 1
    if (eclipse == 1) {
      log_odds <- log_odds + fixef_m4_cond["eclipse_fac1:path_statuswithin_path"]
    }
  }
  
  pred_probs$probability[i] <- plogis(as.vector(log_odds$`(Intercept)`))
}



# Pivot wider to have eclipse=0 and eclipse=1 in separate columns
pred_probs_wide <- pred_probs %>%
  select(species, path_status, eclipse_fac, probability) %>%
  pivot_wider(names_from = eclipse_fac, values_from = probability, 
              names_prefix = "prob_eclipse_") %>%
  # Now add the difference column
  mutate(
    difference = prob_eclipse_1 - prob_eclipse_0,
    direction = ifelse(difference > 0, "Increased", "Decreased"),
    species_path = paste(species, ifelse(path_status == "within_path", "(Within)", "(Outside)"))
  )

# Sort by difference for plotting
pred_probs_wide <- pred_probs_wide %>%
  arrange(difference) %>%
  mutate(species_path = factor(species_path, levels = species_path))

# plot within path

gg_in <- ggplot(pred_probs_wide %>% filter(path_status == 'within_path'), aes(x = species_path, y = difference, fill = direction)) + 
  ...
  labs(y = "Change in Vocalization Activity", x = "Species (Path Status)") + 
  coord_flip() +
  scale_y_continuous(limits = c(-0.1, 0.1)) +  # Set the limits for y-axis
  theme_bw() +
  theme(
...
gg_in
ggsave(filename = here('Results/Figures/change_in_vocal_inside_path.png'),plot = gg_in,device = 'png',width = 8, height = 12, dpi = 300)


gg_out <- ggplot(pred_probs_wide %>% filter(path_status == 'outside_path'), aes(x = species_path, y = difference, fill = direction)) + 
  geom_bar(stat = 'identity', width = 0.5) + 
  scale_fill_manual(name = "Effect of Eclipse", 
                    labels = c("Decreased Vocalization", "Increased Vocalization"), 
                    values = c("Increased" = "black", "Decreased" = "gold")) + 
...
gg_out
ggsave(filename = here('Results/Figures/change_in_vocal_outside_path.png'),plot = gg_out,device = 'png',width = 8, height = 12, dpi = 300)
...
lots_of_dets <- det[, .N, by = family][N >= 50]

filtered_det_fam <- det[family %in% lots_of_dets$family]

m1 <- glmmTMB::glmmTMB(
  y ~ 1 + eclipse_fac + path_status + activity + (1 + eclipse_fac| family),
  data = filtered_det_fam,
  family = binomial(link = "logit")) 


m2 <- glmmTMB::glmmTMB(
  y ~ 1 + eclipse_fac + path_status + (1 + eclipse_fac| family),
  data = filtered_det_fam,
  family = binomial(link = "logit"),
) 
...
m4 <- glmmTMB::glmmTMB(
  y ~ 1 + eclipse_fac*path_status + (1 + eclipse_fac | family) + (1 | station),
  data = filtered_det_fam,
  family = binomial(link = "logit"),
) 
AIC(m1, m2, m3, m4) #m4 is best
...
# Extract fixed and random effects
fixef_m4_cond <- fixef(m4)$cond
ranef_family <- ranef(m4, condVar = TRUE)$cond$family

# Prepare combinations
pred_probs <- expand.grid(
  family = rownames(ranef_family),
  eclipse_fac = c(0, 1),
  path_status = c("within_path", "outside_path"),
  stringsAsFactors = FALSE
)
...
# Loop through and calculate log-odds and probabilities
for (i in 1:nrow(pred_probs)) {
  sp <- pred_probs$family[i]
  eclipse <- pred_probs$eclipse_fac[i]
  path <- pred_probs$path_status[i]
  
  ranef_vals <- ranef_family[sp, ]
  intercept <- ranef_vals[1]
  slope_eclipse <- ranef_vals[2]
...
# Pivot wider to have eclipse=0 and eclipse=1 in separate columns
pred_probs_wide <- pred_probs %>%
  select(family, path_status, eclipse_fac, probability) %>%
  pivot_wider(names_from = eclipse_fac, values_from = probability, 
              names_prefix = "prob_eclipse_") %>%
  # Now add the difference column
  mutate(
    difference = prob_eclipse_1 - prob_eclipse_0,
    direction = ifelse(difference > 0, "Increased", "Decreased"),
    family_path = paste(family, ifelse(path_status == "within_path", "(Within)", "(Outside)"))
  )

# Sort by difference for plotting
pred_probs_wide <- pred_probs_wide %>%
  arrange(difference) %>%
  mutate(family_path = factor(family_path, levels = family_path))

# plot within path

gg_in <- ggplot(pred_probs_wide %>% filter(path_status == 'within_path'), aes(x = family_path, y = difference, fill = direction)) + 
  geom_bar(stat = 'identity', width = 0.5) + 
  scale_fill_manual(name = "Effect of Eclipse", 
                    labels = c("Decreased Vocalization", "Increased Vocalization"), 
                    values = c("Increased" = "black", "Decreased" = "gold")) + 
...
gg_in
ggsave(filename = here('Results/Figures/change_in_vocal_FAMILY_inside_path.png'),plot = gg_in,device = 'png',width = 8, height = 12, dpi = 300)


gg_out <- ggplot(pred_probs_wide %>% filter(path_status == 'outside_path'), aes(x = family_path, y = difference, fill = direction)) + 
  geom_bar(stat = 'identity', width = 0.5) + 
  scale_fill_manual(name = "Effect of Eclipse", 
                    labels = c("Decreased Vocalization", "Increased Vocalization"), 
                    values = c("Increased" = "black", "Decreased" = "gold")) + 
...
gg_out
ggsave(filename = here('Results/Figures/change_in_vocal_FAMILY_outside_path.png'),plot = gg_out,device = 'png',width = 8, height = 12, dpi = 300)
