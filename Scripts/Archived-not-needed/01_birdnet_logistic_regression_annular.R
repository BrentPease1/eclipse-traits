...
focal_time_detections <- in_path |>
  ...
  dplyr::filter( date == ymd("2023-10-12") | date == ymd("2023-10-14")) |> 
...
bw <- readr::read_csv( here::here("Data/North_Ameica_oct2023.csv"))
...
combos <- bw |> 
  ...
  dplyr::cross_join(
    tibble::tibble(
      date = seq(from = lubridate::ymd("2023-10-01"), to = lubridate::ymd("2023-10-21"), by = 1)))
...
clean <- all_data |> 
  ...
  dplyr::filter(!date_local == ymd("2023-10-14")) |> 
...
unique_sites <- birdnet |> 
  dplyr::filter(date == lubridate::ymd("2023-10-14") | date == lubridate::ymd("2023-10-12")) |> 
  dplyr::select(lat = Latitude, lon = Longitude, MaxEclipseTimeUTC) |> 
  dplyr::distinct() |> 
  tibble::add_column(date = lubridate::ymd("2023-10-14")) |> 
...
global_grid_5 <- sf::st_make_grid(sf::st_as_sfc(bounding_box),
                                  cellsize = c(1, 1), crs = sf::st_crs(4326), what = "polygons")
...
ggsave(
  filename = "figure_02a_annular.png", 
  width = 3.5, 
  height = 1.1,
  units = "in", 
  dpi = 600)
...
setwd(here::here("Results/Figures"))
ggsave(
  filename = "figure_02c_annular.png", 
  width = 3.5, 
  height = 7, 
  units = "in",
  dpi = 600)
...
dnb_plot + dnc_plot + cd_plot + hd_plot + mi_plot + 
  nt_plot + lp_plot + et_plot +  plot_layout(ncol = 2,
                                             axis_titles = "collect")

setwd(here::here("Results/Figures"))
ggsave(
  filename = "figure_02b_annular.png", 
  width = 4, 
  height = 6, 
  units = "in",
  dpi = 600)
