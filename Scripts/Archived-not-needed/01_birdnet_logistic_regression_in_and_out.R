...
focal_time_detections <- birdnet[
  ...
  .(station = ESIDNumber, common = common_name, sci = scientific_name, date, activity,
    family, path_status = within_path)]
...
template <- unique(birdnet[date %in% ymd(c("2024-04-07", "2024-04-08", "2024-04-09")), 
                              .(station = ESIDNumber, common = common_name, 
                                sci = scientific_name, activity, family, path_status = within_path)])
...
lots_of_dets <- det[, .N, by = common][N >= 10]
...
lots_of_dets <- det[, .N, by = family][N >= 10]
